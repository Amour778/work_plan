...
public class MailHandler {
	...
	public Map<String, String> sendWCPEMail(String item_id,String approval_sta) {
		...
		try {
			...
			MESSAGE.put("code", "1");
			MESSAGE.put("info", "获取项目名称失败："+e.getMessage());
			return MESSAGE;
		}
		...
		if(user_role.equals("NULL")){
			...
			try {
				...
				MESSAGE.put("code", "1");
				MESSAGE.put("info", "用户角色代码为普通立项用户，获取项目责任人邮箱地址失败："+e.getMessage());
				return MESSAGE;
			}
			...
			try {
				...
				MESSAGE.put("code", "1");
				MESSAGE.put("info", "获取项目所属大区失败："+e.getMessage());
				return MESSAGE;
			}
			...
			try {
				...
				MESSAGE.put("code", "1");
				MESSAGE.put("info", "用户角色代码为大区负责人，获取项目责任人邮箱地址失败："+e.getMessage());
				return MESSAGE;
			}
		}else{
