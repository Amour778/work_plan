layui.define(['laytpl','admin', 'layer', 'element'], function (exports) {
    ...
    var index = {
        ...
        initRouter: function () {
	    	$.ajax({
	            url: projectName+ 'getMenuByUserMenuId',
	            type: 'GET',
	            dataType: 'JSON',
	            success: function (data) {
	        	if(data.length==0){
	        		layer.alert('用户无权限或菜单数据获取失败，请重新登陆', {icon: 5});
	        	}else{
        ...
        bindEvent: function () {
            ...
            $('#setPsw').click(function () {
                admin.popupRight('tpl/password');
            });

            // 个人信息点击事件
            $('#setInfo').click(function () {
            	 admin.popupRight('system/user_form');
            });

            // 消息点击事件
            $('#btnMessage').click(function () {
                //admin.popupRight('tpl/message');
            });
        }
    };
