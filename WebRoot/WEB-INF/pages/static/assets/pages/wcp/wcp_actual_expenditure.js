...
layui.use(['layer','form', 'table','formSelects','admin','notice','dropdown', 'dtree'], function(){
  ...
  var _selectProjectCode=null;
var DemoTree =null;
  formSelects.data('WCPAselect', 'server', {
	    url:"getWCPAGFormSelectsInfo",
  ...
  function formSubmit() {
	  ...
	  DemoTree =dtree.render({
		  ...
		  dot: false,
		  	    ...
		  	    if(data.data.length==0){
		  			  layer.msg("无数据");
		  			  $("#print_wcp_reimbursement").addClass("layui-btn-disabled").attr("disabled", "disabled");
		  		 }else{
		  			$("#print_wcp_reimbursement").removeClass('layui-btn-disabled').removeAttr('disabled');
		  		 }
		  	  },
		  /*toolbar:true,
		  toolbarShow:[], // 默认按钮置空
		  toolbarExt:[
              {toolbarId: "testAdd",icon:"layui-icon layui-icon-print",title:"打印报销单",handler: function(node,$div){
            	  if(node.parentId=="0"){
            		  layer.msg("请选中报销组再打印");
            		  return;
            	  }else{
            		  //admin.putTempData('group_id', node.parentId);
        	    	  admin.putTempData('project_code', _selectProjectCode);
                	  admin.putTempData('group_id', _selectGroupItemId);
            		  admin.popupCenter({
          			    title: '打印-报销单',
          			    path: 'tpl/wcp/print_wcp_reimbursement',
	...
	dtree.on("node('dTree_WCPA')",function(obj) {
		if(obj.param.parentId!=0){
			...
			_selectProjectCode=obj.param.parentId;
		  table.render({
			  	id:'wcp_actual_expenditure_table'
				,toolbar: true
				,loading: true
				,defaultToolbar: ['filter']
				,toolbar: '#wcp_actual_expenditure_toolbar'
			    ,page: false
			    ...
			    ,cols: [[
			      ...
			      ,{field:'edit_remark',  title: '修改备注 ',width:100 }
			      ,{field:'project_area_department',  title: '费用所属部门 ',width:110 }
			    ...
			    ,done: function(res, page, count) {
			    	...
			    	$("[data-field='edit_remark']").children().each(function() {
						if ($(this).text() == 'N') {
							$(this).html('');
						}
					});
	  ...
	  //头工具栏事件
	table.on('toolbar(wcp_actual_expenditure_table_tool)', function(obj){
	    switch(obj.event){
	      case 'print_wcp_reimbursement':
	    	  admin.putTempData('project_code', _selectProjectCode);
        	  admin.putTempData('group_id', _selectGroupItemId);
    		  admin.popupCenter({
    			    title: '打印-报销单',
  ...
  table.on('tool(wcp_actual_expenditure_table_tool)', function (obj) {
      ...
      if (layEvent === 'edit') {
        	...
        	admin.putTempData('project_area_department', data.project_area_department);
			admin.popupCenter({
			    title: '修改报销内容',
